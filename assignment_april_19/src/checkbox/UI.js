        ...
        switch (type) {
            ...
            case 'checkbox': {
                if (!Array.isArray(value)) {
                    ...
                    input = ""
                    value.forEach((val) => {
                        input += `<div class="form-check"><input id=checkbox-"${val.DataValueField}" name=${placholder} class="form-check-input${className.length ? ' ' + className : ""}" type="checkbox" value="${val.DataValueField}" name="${placholder}" ${disabled ? "disabled" : ""}></input><label class="form-check-label" for="${val.DataValueField}">${val.DataTextField}</label></div>`
                    })
                }
                break
            }
