...
function initialize() {
    ...
    loadCartProducts(populateCartTable)
    loadBillRecords(populateBillTable)
    document.getElementById('purchase').addEventListener('click', generateBill, false)    
}
...
function populateCartTable(productList) {
    var rows = CartTable(productList)    
    console.log(rows)
...
function populateBillTable(bills) {
    var rows = BillTable(bills)
    document.getElementById('bill-table').innerHTML = rows
}
